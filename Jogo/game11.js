...
const CONFIG = {
    vidaMaximaChica: 25,          // Quantidade de cliques de lanterna necessários para derrotar a Chica
    vidaMaximaFreddy: 90,         // Vida em pontos decimais do Boss Freddy (reduzida continuamente sob a luz)
    vidaMaximaBonnie: 20,         // Quantidade de cliques na tecla 'Q' com o Taser para derrotar o Bonnie
...
// 2. Atribuindo os caminhos dos arquivos (Adicione isso abaixo)
// Supondo que suas imagens estejam em uma pasta chamada 'assets' ou 'imagens'
sprites.player.src = 'assets/player.png';
sprites.chica.src = 'assets/chica.png';
sprites.bonnie.src = 'assets/bonnie.png';
sprites.freddy.src = 'assets/freddy.png';
sprites.endo.src = 'assets/endo.png';
sprites.pizza.src = 'assets/pizza.png';
sprites.taser.src = 'assets/taser.png';
sprites.moeda.src = 'assets/moeda.png';
sprites.batteryItem.src = 'assets/bateria.png';
sprites.lanternaHUD.src = 'assets/hud_lanterna.png';
sprites.moedaHUD.src = 'assets/hud_moeda.png';
...
// ========================================================
// Executa a transição física entre salas, limpando lixos da GPU, reposicionando o player e gerando novos monstros
function carregarSala(numero, vindoDeOnde = "baixo") {
    // Reinicializa todos os arrays de entidades temporárias da sala anterior para liberar memória
    ...
    // Sincroniza estados de eliminação dos chefes globais
    ...
    // Determina o ponto geométrico de surgimento do jogador baseado no portal que ele atravessou
    if (vindoDeOnde === "baixo") { player.x = 475; player.y = 800; }        // Entrando pelo sul (subindo de nível)
    else if (vindoDeOnde === "cima") { player.x = 475; player.y = 80; }     // Entrando pelo norte (recuando de nível)
    else if (vindoDeOnde === "esquerda") { player.x = 850; player.y = 425; } // Voltando da loja lateral esquerda
    else if (vindoDeOnde === "direita") { player.x = 100; player.y = 425; }  // Saindo da loja em direção à arena 4

    // Mecânicas de rebalanceamento imediato ao adentrar as salas dos chefes
    if (numero === 3 && chica.vivo) { player.battery = 100; itemPerkChicaNoChao = null; } // Recarrega bateria para a batalha da Chica
    if (numero === 6 && bonnie.vivo) { bonnie.ultimoAtaque = Date.now() + 1500; bonnie.atordoado = false; bonnie.timerAtordoado = 0; itemPerkBonnieNoChao = null; } // Dá tempo de reação contra o Bonnie
    if (numero === 8 && player.battery < 45) { player.battery = 75; } // Fornece carga assistida mínima contra o Freddy
    ...
    if (salaComInimigosAtivos && inimigos.length === 0) {
        let quantidadeInimigos = (numero === 4) ? 6 : 3; // Sala 4 (ninho) spawna o dobro de monstros
        
        for(let i = 0; i < quantidadeInimigos; i++) {
            let tipoSorteado = Math.floor(Math.random() * 5) + 1; // Classifica a IA do Endo aleatoriamente entre os tipos 1 e 5
            let posicaoValida = false;
            let tentativaX, tentativaY;
            let tentativasMaximas = 50; // Proteção contra travar o loop caso falte espaço físico

            // Loop de validação de posicionamento (Raycast / Proximidade preventiva)
            while(!posicaoValida && tentativasMaximas > 0) {
                tentativaX = Math.random() * 800 + 100; // Restringe às bordas internas da parede lateral
                tentativaY = Math.random() * 500 + 100; // Restringe às bordas internas superiores/inferiores
                tentativasMaximas--;

                // Impede o monstro de nascer em cima do jogador (Distância mínima de 200 pixels)
                let distPlayer = Math.hypot(tentativaX - player.x, tentativaY - player.y);
                if (distPlayer < 200) continue;

                // Impede o monstro de nascer colado a um companheiro de equipe (Espaçamento mínimo de 75 pixels)
                ...
                if(!muitoPertoDeOutro) {
                    posicaoValida = true; // Local seguro aprovado pelo motor matemático
                }
            }

            // Adiciona a entidade instanciada na matriz ativa de monitoramento do jogo
            inimigos.push({
    ...
    // Se o jogador possui a insígnia da Chica, gera aleatoriamente fatias de pizza de buffs na sala
    if (player.temPerkChica && (salaComInimigosAtivos || [3, 6, 8].includes(numero))) {
        let qtdPizzas = Math.floor(Math.random() * 2) + 1; // Spawna de 1 a 2 pizzas utilitárias
        for(let p = 0; p < qtdPizzas; p++) {
            pizzasEspeciaisNoChao.push({
                ...
                w: 35,
                h: 35,
                tipo: Math.random() < 0.5 ? "velocidade" : "escudo" // Divide entre fatias Douradas ou Azuis
            });
        }
    }

    // Executa a ativação retardada dos perks de inventário associados à nova sala carregada
...
function atualizar() {
    ...
    // Avalia se ainda restam robôs agressivos que precisam ser abatidos para liberar as portas trancadas da sala
